...
async function main() {
  ...
  const memorialEvent = await prisma.event.create({
    data: {
      userId: user.id,
      category: events_category.FUNERAL,
      ...
      eventHosts: {
        create: [
          {
            name: '김준혁',
            role: eventhost_role.DEAD,
          },
          {
            name: '김민수',
            role: eventhost_role.CHIEF_MOURNER,
            accounts: {
              create: [
                {
                  bank: '하나은행',
  ...
  const weddingEvent = await prisma.event.create({
    data: {
      userId: user.id,
      category: events_category.WEDDING,
